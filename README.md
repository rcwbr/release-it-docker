...
{
  "extends": [
    "github:rcwbr/release-it-docker/base#0.8.0"
  ]
}
```
...
```bash
docker run -it ghcr.io/rcwbr/release-it-docker:0.8.0
```
...
{
  "extends": [
    "github:rcwbr/release-it-docker/conventional-changelog#0.8.0"
  ]
}
```
...
```bash
docker run -it ghcr.io/rcwbr/release-it-docker-conventional-changelog:0.8.0
```
...
```bash
docker run -it -v $(pwd):$(pwd) -w $(pwd) ghcr.io/rcwbr/release-it-docker-file-bumper:0.8.0
```
...
{
  "extends": [
    "github:rcwbr/release-it-docker/file-bumper/tag-only#0.8.0"
  ]
}
```
...
```bash
docker run -it -v /var/run/docker.sock:/var/run/docker.sock -v $(pwd):$(pwd) -w $(pwd) ghcr.io/rcwbr/release-it-docker-file-bumper:0.8.0
```
